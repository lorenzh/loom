...
| 2026-04-05 | **Timer is ref'd — watcher owns process lifetime.** Removed `timer.unref()`. The polling timer now keeps the host process alive while the watcher is active. Required for one-shot (`--stdin`) mode where no other handles exist; without it the process exits before the first poll tick fires. Callers must call `stop()` to allow the process to exit. |
